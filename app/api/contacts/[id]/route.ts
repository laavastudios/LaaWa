...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const contact = await query(
    `SELECT c.id,c.name,c.push_name,c.phone,c.email,c.notes,c.metadata,c.created_at,c.updated_at,
     ...
     WHERE c.id=$1 AND c.workspace_id=$2 LIMIT 1`, [id, wid]
  );
  if (!contact.rows[0]) return NextResponse.json({ error: "Contact not found." }, { status: 404 });

  const tags = await query(`SELECT t.id,t.name FROM tags t JOIN contact_tags ct ON ct.tag_id=t.id WHERE ct.contact_id=$1 AND t.workspace_id=$2 ORDER BY t.name`, [id, wid]);
  const conversations = await query(
    `SELECT v.id,v.chat_id,v.title,v.status,v.unread_count,v.last_message_at,v.last_message_preview,v.created_at
     FROM conversations v WHERE v.contact_id=$1 AND v.workspace_id=$2 ORDER BY v.last_message_at DESC NULLS LAST LIMIT 20`, [id, wid]
  );
  const messages = await query(
    `SELECT m.id,m.conversation_id,m.direction,m.message_type,m.body,m.status,m.created_at,m.external_timestamp
     ...
     WHERE v.contact_id=$1 AND v.workspace_id=$2 ORDER BY m.created_at DESC LIMIT 60`, [id, wid]
  );

  return NextResponse.json({ configured: true, contact: contact.rows[0], tags: tags.rows, conversations: conversations.rows, messages: messages.rows });
}
