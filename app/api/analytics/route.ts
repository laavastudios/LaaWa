...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [messages, daily, accounts, contacts, conversations, broadcasts, automation, jobs, events] = await Promise.all([
      query<any>(`SELECT COUNT(*)::int total, COUNT(*) FILTER (WHERE m.direction='inbound')::int inbound, COUNT(*) FILTER (WHERE m.direction='outbound')::int outbound, COUNT(*) FILTER (WHERE m.status='failed')::int failed, COUNT(*) FILTER (WHERE m.status IN ('sent','delivered','read'))::int delivered FROM messages m JOIN conversations c ON c.id=m.conversation_id WHERE c.workspace_id=$1 AND m.created_at >= ${since}`, [wid, days]),
