...
function start(account, automatic = false) {
  ...
  intentionalStops.delete(account.id);
  ...
  child.on("exit", (code, signal) => {
    ...
    const intentional = intentionalStops.delete(account.id);
...
function scheduleRestart(account) {
  const now = Date.now();
  const state = restartState.get(account.id) || { startedAt: now, attempts: 0, timer: null };
...
function stop(account) { const state = restartState.get(account.id); if (state?.timer) clearTimeout(state.timer); restartState.delete(account.id); intentionalStops.add(account.id); const child = children.get(account.id); if (!child) { intentionalStops.delete(account.id); return; } try { child.kill(); } catch {} children.delete(account.id); emit("account", { id: account.id, status: "stopped" }); }
