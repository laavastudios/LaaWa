...
const spec = {
  ...
  info: {
    title: "LaaWa API",
    version: "1.0.0",
    description: "Versioned API for messaging, conversations, contacts, API keys, and realtime events.",
  },
  ...
  components: {
    securitySchemes: {
      bearerAuth: { type: "http", scheme: "bearer", description: "Use a LaaWa API key. Session authentication is also supported for browser requests." },
    },
    schemas: {
  ...
  paths: {
    ...
    "/keys": {
      get: { tags: ["API Keys"], summary: "List API keys", security: [{ bearerAuth: [] }], responses: { "200": { description: "API keys" }, "401": { description: "Unauthorized" } } },
      post: { tags: ["API Keys"], summary: "Create API key", security: [{ bearerAuth: [] }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["name"], properties: { name: { type: "string" }, scopes: { type: "array", items: { type: "string", enum: ["read", "write", "admin"] } }, whatsappAccountIds: { type: "array", items: { type: "string", format: "uuid" } }, expiresAt: { type: "string", format: "date-time" } } } } } }, responses: { "201": { description: "Created key; plaintext is returned once" }, "400": { description: "Invalid request" } } },
    },
    ...
    "/messages": {
      get: { tags: ["Messages"], summary: "Read chat messages", parameters: [{ name: "accountId", in: "query", required: true, schema: { type: "string" } }, { name: "chatId", in: "query", required: true, schema: { type: "string" } }], responses: { "200": { description: "Messages" } } },
      post: { tags: ["Messages"], summary: "Send or act on a message", requestBody: { required: true, content: { "application/json": { schema: { type: "object", properties: { action: { type: "string", enum: ["send", "location", "contact", "reply", "react", "read"], default: "send" }, accountId: { type: "string" }, chatId: { type: "string" }, conversationId: { type: "string" }, messageId: { type: "string" }, text: { type: "string" }, reaction: { type: "string" }, latitude: { type: "number" }, longitude: { type: "number" }, description: { type: "string" }, phone: { type: "string" }, name: { type: "string" }, vcard: { type: "string" }, media: { type: "object" } } } } } }, responses: { "201": { description: "Message action accepted" }, "400": { description: "Invalid request" }, "403": { description: "Insufficient scope" } } },
    },
