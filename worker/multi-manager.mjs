...
function start(account, automatic = false) {
  ...
  child.on("exit", (code, signal) => {
    ...
    emit("account", { id: account.id, status: code === 0 ? "stopped" : "error", code, signal });
    if (code !== 0 && accounts.some((item) => item.id === account.id)) scheduleRestart(account);
  });
...
function stop(account) { const state = restartState.get(account.id); if (state?.timer) clearTimeout(state.timer); restartState.delete(account.id); const child = children.get(account.id); if (!child) return; try { child.kill(); } catch {} children.delete(account.id); emit("account", { id: account.id, status: "stopped" }); }
