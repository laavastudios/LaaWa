...
export async function GET(request: NextRequest) {
  ...
  try {
    const [policy, summary] = await Promise.all([getPrivacyPolicy(workspaceId), getPrivacySummary(workspaceId)]);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const policy = await updatePrivacyPolicy(workspaceId, input);
...
export async function POST(request: NextRequest) {
  ...
  if (body.action === "retention") {
    try {
      return apiSuccess(await runPrivacyRetention(workspaceId), { requestId: auth.requestId });
    } catch (error) {
  ...
  if (body.action === "erase") {
    const summary = await getPrivacySummary(workspaceId);
    ...
    try {
      const result = await eraseWorkspaceData(workspaceId, auth.principal.id);
