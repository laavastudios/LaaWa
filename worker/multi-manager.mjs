...
function loadAccounts() {
  try {
    ...
    if (!Array.isArray(value) || !value.length) throw new Error("invalid config");
    return value.filter((x) => x?.id && Number(x?.port) && x?.authPath).slice(0, MAX_ACCOUNTS);
  } catch { return [{ id: "default", name: "Primary WhatsApp", port: BASE_PORT, authPath: BASE_AUTH }]; }
}
...
function start(account) {
  ...
  child.on("exit", (code, signal) => { children.delete(account.id); emit("account", { id: account.id, status: code === 0 ? "stopped" : "error", code, signal }); });
  emit("account", { id: account.id, status: "starting" });
}
...
async function workerRequest(account, pathname, options = {}) {
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 3500);
  try {
    const response = await fetch(`http://127.0.0.1:${account.port}${pathname}`, { ...options, cache: "no-store", signal: controller.signal, headers: { ...(options.headers || {}), ...(SECRET ? { Authorization: `Bearer ${SECRET}` } : {}) } });
    return { status: response.status, data: await response.json().catch(() => ({})) };
  } catch { return { status: 503, data: { status: "offline", connected: false, error: "Worker unavailable" } }; }
  finally { clearTimeout(timer); }
}
async function snapshot() {
  return Promise.all(accounts.map(async (account) => {
    const response = await workerRequest(account, "/status");
    return { ...account, status: response.data.status || "offline", connected: Boolean(response.data.connected), phone: response.data.phone || null, profileName: response.data.name || null, error: response.data.error || null, process: children.has(account.id) ? "running" : "stopped" };
  }));
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/events") {
    ...
    const timer = setInterval(async () => { try { res.write(`event: snapshot\ndata: ${JSON.stringify({ accounts: await snapshot() })}\n\n`); } catch {} }, 5000);
    req.on("close", () => { clearInterval(timer); subscribers.delete(res); }); return;
  }
  ...
  if (req.method === "POST" && url.pathname === "/accounts") {
    if (accounts.length >= MAX_ACCOUNTS) return json(res, 409, { error: `Maximum of ${MAX_ACCOUNTS} WhatsApp accounts reached.` });
    let body = ""; for await (const chunk of req) body += chunk;
    let payload; try { payload = JSON.parse(body || "{}"); } catch { return json(res, 400, { error: "Invalid JSON." }); }
    const id = cleanId(payload.id || payload.name); if (!id) return json(res, 400, { error: "Account id is required." });
    if (accountFor(id)) return json(res, 409, { error: "That account id already exists." });
    const usedPorts = new Set(accounts.map((account) => Number(account.port))); let port = BASE_PORT + 1; while (usedPorts.has(port)) port += 1;
    const account = { id, name: String(payload.name || id).trim().slice(0, 80) || id, port, authPath: path.resolve(`./.whatsapp-session-${id}`) };
    accounts.push(account); saveAccounts(); start(account); return json(res, 201, { account: { ...account, status: "starting", connected: false, process: "running" } });
  }
  const match = url.pathname.match(/^\/accounts\/([^/]+)(?:\/(restart|stop|status))?$/);
  if (match) {
    const account = accountFor(cleanId(match[1])); if (!account) return json(res, 404, { error: "WhatsApp account not found." });
    const action = match[2] || "status";
    if (action === "restart") { stop(account); start(account); return json(res, 202, { ok: true, id: account.id, status: "starting" }); }
    if (action === "stop") { stop(account); return json(res, 200, { ok: true, id: account.id, status: "stopped" }); }
    return json(res, 200, (await workerRequest(account, "/status")).data);
  }
  if (req.method === "DELETE" && match) {
    const account = accountFor(cleanId(match[1])); if (!account) return json(res, 404, { error: "WhatsApp account not found." });
    if (account.id === "default") return json(res, 400, { error: "The primary account cannot be removed." });
    stop(account); accounts = accounts.filter((item) => item.id !== account.id); saveAccounts(); return json(res, 200, { ok: true });
  }
