...
function workerUrl() {
  return process.env.WHATSAPP_WORKER_URL || "http://127.0.0.1:3010";
}
...
function conditionsMatch(conditions: unknown, body: string) {
  ...
  return conditions.every((condition) => {
    ...
    switch (item.type) {
      ...
      case "contains":
      default: return text.includes(value);
    }
  });
}
...
function replyActions(actions: unknown) {
  ...
  return actions
    .filter((action): action is { type?: string; body?: string } => Boolean(action && typeof action === "object"))
    .filter((action) => !action.type || action.type === "reply")
    .map((action) => String(action.body || "").trim())
    .filter(Boolean);
}
...
async function context() {
  ...
  const account = await query<{ id: string }>(
    "SELECT id FROM whatsapp_accounts WHERE workspace_id = $1 ORDER BY created_at ASC LIMIT 1",
    [workspace.rows[0].id],
  );
...
export async function evaluateAutomations(message: IncomingMessage) {
  ...
  for (const rule of rules.rows) {
    if (rule.trigger_type !== "message") continue;
    if (!conditionsMatch(rule.conditions, String(message.body))) continue;

    const conversation = await query<{ id: string }>(
      "SELECT id FROM conversations WHERE whatsapp_account_id = $1 AND chat_id = $2 LIMIT 1",
      [ctx.accountId, message.chatId],
    );
    const conversationId = conversation.rows[0]?.id || null;
    const run = await query<{ id: string }>(
      `INSERT INTO automation_runs (rule_id, conversation_id, status, result)
       VALUES ($1, $2, 'running', $3::jsonb) RETURNING id`,
      [rule.id, conversationId, JSON.stringify({ matched: true, trigger: message.body })],
    );
    ...
    try {
      ...
      for (const body of replies) {
        const response = await fetch(`${workerUrl()}/send`, {
          ...
          body: JSON.stringify({ chatId: message.chatId, body }),
        });
      ...
      await query(
        `UPDATE automation_runs
         SET status = 'completed', result = result || $2::jsonb, completed_at = NOW()
         WHERE id = $1`,
        [run.rows[0].id, JSON.stringify({ sent })],
      );
    } catch (error) {
      await query(
        `UPDATE automation_runs SET status = 'failed', error = $2, completed_at = NOW() WHERE id = $1`,
