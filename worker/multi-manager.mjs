...
const server = http.createServer(async (req, res) => {
  ...
  if (match) { if (!account) return json(res, 404, { error: "WhatsApp account not found." }); const action = match[2] || "status"; if (action === "restart") { stop(account); start(account); return json(res, 202, { ok: true, id: account.id, status: "starting" }); } if (action === "stop") { stop(account); return json(res, 200, { ok: true, id: account.id, status: "stopped" }); } if (action === "send") { let body = ""; for await (const chunk of req) body += chunk; let payload; try { payload = JSON.parse(body || "{}"); } catch { return json(res, 400, { error: "Invalid JSON." }); } return json(res, (await workerRequest(account, "/send", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) })).status, (await workerRequest(account, "/send", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) })).data); } return json(res, 200, (await workerRequest(account, "/status")).data); }
