...
export default function AnalyticsHub(){
 ...
 async function load(){setLoading(true);setError("");try{const r=await fetch(`/api/analytics?days=${days}`,{cache:"no-store"});const d=await r.json();if(!r.ok)throw new Error(d.error||"Analytics unavailable");setData(d)}catch(e){setError(e instanceof Error?e.message:"Analytics unavailable")}finally{setLoading(false)}}
 useEffect(()=>{load()},[days]);
 ...
 return <div className="analytics-wrap">
  ...
  {!data.configured&&<div className="empty glass"><strong>Database not configured</strong><span>Connect PostgreSQL to see live workspace analytics.</span></div>}
  {data.configured&&<>
