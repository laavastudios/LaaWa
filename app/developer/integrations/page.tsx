...
export default function IntegrationsPage() {
  ...
  async function create() {
    if (!name.trim()) return; setBusy(true); setMessage("");
    const r = await fetch("/api/v1/integrations", { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({ provider:selected, name:name.trim(), config }) });
    const b = await r.json(); setBusy(false); if (!r.ok) { setMessage(b?.error?.message || "Unable to save integration."); return; } setName(""); setConfig({}); setMessage("Integration secured. Test it from the connection card."); await load();
  }
  ...
  return <main className="integration-shell">
    <div className="integration-grid-glow" /><div className="integration-orb orb-a" /><div className="integration-orb orb-b" />
    ...
    <section className="integration-layout">
      <article className="integration-card builder"><div className="card-heading"><span className="integration-icon"><Zap size={18}/></span><div><h2>New connection</h2><p>Secrets stay server-side and encrypted at rest.</p></div></div>
        ...
        <button className="integration-primary" disabled={busy||!name.trim()} onClick={create}><Link2 size={16}/>{busy?"Securing…":"Secure connection"}</button>
      </article>
