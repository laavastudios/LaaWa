...
function workspaceName() {
  return process.env.LAAWA_WORKSPACE_NAME || "LaaWa Workspace";
}
...
async function ensureContext() {
  ...
  const account = await query<{ id: string }>(
    "SELECT id FROM whatsapp_accounts WHERE workspace_id = $1 AND session_key = $2 LIMIT 1",
    [workspaceId, accountKey()],
  );
  const accountId = account.rows[0]?.id || (
    await query<{ id: string }>(
      "INSERT INTO whatsapp_accounts (workspace_id, name, session_key) VALUES ($1, $2, $3) RETURNING id",
      [workspaceId, "WhatsApp", accountKey()],
    )
  ).rows[0].id;
...
function messageType(type?: string) {
  ...
  if (["text", "image", "video", "audio", "document", "sticker", "location", "contact"].includes(value)) return value;
  ...
  for (const message of input) {
    ...
    if (!chatId.endsWith("@g.us") && !chatId.includes("@broadcast")) {
      const contact = await query<{ id: string }>(
        `INSERT INTO contacts (workspace_id, whatsapp_account_id, wa_id, phone, name, avatar_url)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (whatsapp_account_id, wa_id) WHERE wa_id IS NOT NULL
         DO UPDATE SET phone = COALESCE(EXCLUDED.phone, contacts.phone), name = COALESCE(NULLIF(EXCLUDED.name, ''), contacts.name), avatar_url = COALESCE(EXCLUDED.avatar_url, contacts.avatar_url), updated_at = NOW()
         RETURNING id`,
        [workspaceId, accountId, chatId, phone, name, message.avatar || null],
      );
      contactId = contact.rows[0]?.id || null;
    }

    const conversation = await query<{ id: string }>(
      `INSERT INTO conversations (workspace_id, whatsapp_account_id, contact_id, chat_id, chat_type, title, unread_count, last_message_at, last_message_preview)
       VALUES ($1, $2, $3, $4, $5, $6, $7, to_timestamp($8), $9)
       ON CONFLICT (whatsapp_account_id, chat_id)
       ...
       RETURNING id`,
      [workspaceId, accountId, contactId, chatId, chatType, name, message.fromMe || message.read ? 0 : 1, timestamp, body || (message.hasMedia ? `[${type}]` : "[message]")],
    );
    const conversationId = conversation.rows[0]?.id;
    if (!conversationId) continue;

    await query(
      `INSERT INTO messages (conversation_id, whatsapp_account_id, whatsapp_message_id, direction, sender_wa_id, recipient_wa_id, message_type, body, status, external_timestamp, metadata)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, to_timestamp($10), $11::jsonb)
