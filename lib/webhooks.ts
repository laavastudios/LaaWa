...
async function deliver(id: string, url: string, secret: string, eventType: string, payload: Record<string, unknown>) {
  ...
  return { delivered: false, attempts: RETRIES, status: lastStatus };
}
...
export async function emitWebhookEvent(workspaceId: string, eventType: string, payload: Record<string, unknown>, webhookId?: string) {
  if (!isDatabaseConfigured()) return { queued: 0, delivered: 0, failed: 0 };
  ...
  const results = await Promise.all(hooks.rows.map(async (hook) => {
    if (!hook.secret_ciphertext) return { queued: 0, delivered: 0, failed: 1 };
    try {
      ...
      const result = await deliver(delivery.rows[0].id, hook.url, decryptWebhookSecret(hook.secret_ciphertext), eventType, payload);
      return result.delivered ? { queued: 1, delivered: 1, failed: 0 } : { queued: 1, delivered: 0, failed: 1 };
    } catch (error) { console.error("Webhook delivery failed:", error instanceof Error ? error.message : String(error)); return { queued: 0, delivered: 0, failed: 1 }; }
  }));
  return results.reduce((total, result) => ({ queued: total.queued + result.queued, delivered: total.delivered + result.delivered, failed: total.failed + result.failed }), { queued: 0, delivered: 0, failed: 0 });
}
