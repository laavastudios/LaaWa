...
export async function GET() {
  ...
  try {
    ...
    const rules = await query(
      `SELECT id, name, enabled, trigger_type, conditions, actions, created_at, updated_at
       FROM automation_rules WHERE workspace_id = $1 ORDER BY created_at DESC`,
      [workspace],
    );
    const runs = await query(
      `SELECT ar.id, ar.rule_id, ar.status, ar.result, ar.error, ar.created_at, r.name
       FROM automation_runs ar JOIN automation_rules r ON r.id = ar.rule_id
       WHERE r.workspace_id = $1 ORDER BY ar.created_at DESC LIMIT 50`,
      [workspace],
    );
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const match = String(body?.match || "contains").trim();
    ...
    const result = await query(
      `INSERT INTO automation_rules (workspace_id, name, trigger_type, conditions, actions)
       VALUES ($1, $2, 'message', $3::jsonb, $4::jsonb)
       RETURNING id, name, enabled, trigger_type, conditions, actions, created_at, updated_at`,
      [workspace, name, JSON.stringify([{ type: match, value: keyword }]), JSON.stringify([{ type: "reply", body: reply }])],
    );
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!workspace) return NextResponse.json({ error: "Workspace is not initialized." }, { status: 503 });
    const result = await query(
      `UPDATE automation_rules SET enabled = COALESCE($3, enabled), updated_at = NOW()
       WHERE id = $1 AND workspace_id = $2 RETURNING id, name, enabled, trigger_type, conditions, actions, created_at, updated_at`,
      [id, workspace, typeof body?.enabled === "boolean" ? body.enabled : null],
    );
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to delete automation." }, { status: 500 });
  }
}
