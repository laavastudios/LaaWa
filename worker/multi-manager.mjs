...
function scheduleRestart(account) {
  const now = Date.now();
  const existing = restartState.get(account.id);
...
function stop(account) {
  const state = restartState.get(account.id);
  if (state?.timer) clearTimeout(state.timer);
  restartState.delete(account.id);
  const child = children.get(account.id);
  if (!child) return;
  intentionalStops.add(child);
  try { child.kill(); } catch {}
  children.delete(account.id);
  emit("account", { id: account.id, status: "stopped" });
}
